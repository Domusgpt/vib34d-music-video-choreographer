...
## Storage Resilience & Session Feedback
- Added a `StorageBridge` utility that probes browser persistence, downgrades to in-memory storage on quota failures, mirrors the `localStorage` API, and emits mode plus change events so choreographer modules stay responsive even in private browsing contexts.【F:src/core/StorageBridge.js†L1-L260】
- Refactored the choreographer to route scene sets and automation snapshots through the bridge, surface session-only messaging, and warn performers when persistent saves are unavailable while keeping snapshot info and UI status tags in sync.【F:music-choreographer-engine.js†L40-L164】【F:music-choreographer-engine.js†L1888-L2050】【F:music-choreographer-engine.js†L3868-L3957】
- Updated the control surface shell to rely on the bridge for panel preferences, exposing a live storage indicator pill that reflects persistent, session-only, or offline modes so performers immediately know whether their layout choices will survive a reload.【F:index.html†L1995-L2051】
- Ensured fallback sessions retain their captured data when persistence returns by mirroring storage entries into the memory adapter, exposing a restore API, and wiring focus/online/visibility probes across the engine and control panel to retry persistence and reflect the upgraded mode immediately.【F:src/core/StorageBridge.js†L261-L337】【F:music-choreographer-engine.js†L1931-L1969】【F:index.html†L2013-L2051】
- Added cross-tab storage synchronisation so external saves, clears, and preference edits propagate live to the scene lab, automation snapshots, and control panel, keeping multiple tabs aligned without manual refreshes.【F:src/core/StorageBridge.js†L163-L337】【F:music-choreographer-engine.js†L1792-L1928】【F:index.html†L2007-L2137】
- Layered a BroadcastChannel handshake that mirrors mutations, shares session snapshots, and suppresses duplicate storage events so memory-only fallbacks stay synchronised across open tabs without requiring persistent storage.【F:src/core/StorageBridge.js†L17-L204】【F:src/core/StorageBridge.js†L261-L401】
...
- Added a recovery flow Playwright test that forces storage writes to fail, captures a scene in session mode, re-enables persistence, and reloads to verify the restored set survives in localStorage with the UI indicators reflecting the upgrade.【F:tests/storage-recovery.test.js†L1-L111】
- Verified cross-tab synchronisation by scripting a Playwright scenario that opens auxiliary tabs, edits and clears stored scene sets, and asserts the primary control surface reflects remote updates in real time.【F:tests/storage-cross-tab-sync.test.js†L1-L107】
- Added a broadcast regression that forces memory fallback, captures a scene, and proves secondary tabs receive the snapshot plus subsequent renames through the new channel synchronisation.【F:tests/storage-memory-broadcast.test.js†L1-L92】
