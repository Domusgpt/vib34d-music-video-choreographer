...
## Mobile Ergonomics & Visualizer Lifecycle
- Added a canvas lifecycle manager inside the choreographer that releases WebGL contexts before system switches, recreates appropriately sized canvases, keeps a resize observer in sync with mobile viewport changes, and exposes a full `destroy()` teardown so live mode toggles never leak GPU resources.【F:music-choreographer-engine.js†L200-L365】【F:music-choreographer-engine.js†L4584-L4651】【F:music-choreographer-engine.js†L5283-L5348】
- Queued choreographer initialization in the shell so repeated mode toggles on touch devices await smart destruction before spinning up the next engine, preventing race conditions while rehydrating the control surface.【F:index.html†L1875-L1906】【F:index.html†L2339-L2362】
- Tuned the control surface height calculations to leverage `visualViewport`, added viewport resize/scroll listeners, and preserved responsive collapse behaviour for keyboard popovers, delivering a steadier drawer on tablets and phones.【F:index.html†L2084-L2144】【F:index.html†L2268-L2299】
