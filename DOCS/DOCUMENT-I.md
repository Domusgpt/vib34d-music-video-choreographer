...
- Added a `StorageBridge` utility that probes browser persistence, downgrades to in-memory storage on quota failures, mirrors the `localStorage` API, and emits mode-change events so choreographer modules stay responsive even in private browsing contexts.【F:src/core/StorageBridge.js†L1-L184】
- Refactored the choreographer to route scene sets and automation snapshots through the bridge, surface session-only messaging, and warn performers when persistent saves are unavailable while keeping snapshot info and UI status tags in sync.【F:music-choreographer-engine.js†L47-L76】【F:music-choreographer-engine.js†L1673-L1768】【F:music-choreographer-engine.js†L1877-L2013】【F:music-choreographer-engine.js†L3839-L3883】
- Updated the control surface shell to rely on the bridge for panel preferences, exposing a live storage indicator pill that reflects persistent, session-only, or offline modes so performers immediately know whether their layout choices will survive a reload.【F:index.html†L1707-L1722】【F:index.html†L1918-L1993】
- Ensured fallback sessions retain their captured data when persistence returns by mirroring storage entries into the memory adapter, exposing a restore API, and wiring focus/online/visibility probes across the engine and control panel to retry persistence and reflect the upgraded mode immediately.【F:src/core/StorageBridge.js†L1-L322】【F:music-choreographer-engine.js†L40-L205】【F:music-choreographer-engine.js†L1783-L1821】【F:music-choreographer-engine.js†L4494-L4511】【F:index.html†L1967-L2044】
