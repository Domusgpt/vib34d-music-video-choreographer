...
import { LiveMacroManager } from './src/core/LiveMacroManager.js';
...
export class MusicVideoChoreographer {
    constructor(mode = 'reactive') {
        ...
        this.macroManager = new LiveMacroManager({ slotCount: 6, blendDuration: 1600 });
        this.currentMacroLabel = 'Live Pad';
        this.activeMacroSlot = null;
        this.macroStatusTimeout = null;
        this.defaultMacroMessage = 'Shift+Click to store, Click to recall performance macros.';
        this.xyPad = null;
    ...
    cacheDomReferences() {
        this.dom = {
            ...
            macroButtons: Array.from(document.querySelectorAll('[data-macro-slot]')),
            macroStatus: document.getElementById('macro-status'),
            macroMorphSlider: document.getElementById('macro-morph-slider'),
            macroMorphValue: document.getElementById('macro-morph-value'),
    ...
    setupMacroControls() {
        if (!this.macroManager) return;

        if (!Array.isArray(this.dom.macroButtons)) {
            this.dom.macroButtons = Array.from(document.querySelectorAll('[data-macro-slot]'));
        }

        const { macroButtons, macroMorphSlider, macroMorphValue } = this.dom;

        if (macroMorphSlider) {
            const initialDuration = this.macroManager.getBlendDuration();
            macroMorphSlider.value = initialDuration;
            if (macroMorphValue) {
                macroMorphValue.textContent = this.formatMacroDuration(initialDuration);
            }
            macroMorphSlider.addEventListener('input', (event) => {
                const next = this.macroManager.setBlendDuration(event.target.value);
                if (macroMorphValue) {
                    macroMorphValue.textContent = this.formatMacroDuration(next);
                }
                this.updateMacroStatus(`Morph duration: ${this.formatMacroDuration(next)}`, 'info', 1200);
            });
        }

        if (Array.isArray(macroButtons) && macroButtons.length) {
            macroButtons.forEach((button) => {
                const index = Number(button.dataset.macroSlot);
                if (!Number.isFinite(index)) return;

                button.addEventListener('click', (event) => {
                    if (event.shiftKey || event.altKey || event.metaKey) {
                        this.captureMacroSlot(index);
                    } else {
                        this.recallMacroSlot(index);
                    }
                });
                ...
                button.addEventListener('dblclick', (event) => {
                    event.preventDefault();
                    this.promptRenameMacro(index);
                });
            });
        }

        this.clearActiveMacro(this.currentMacroLabel || 'Live Pad');
        this.refreshMacroButtons();
        this.updateMacroStatus(this.defaultMacroMessage, 'muted', false);
    }

    formatMacroDuration(value) {
        const duration = Math.max(0, Number(value) || 0);
        return `${(duration / 1000).toFixed(2)}s`;
    }

    formatMacroTimestamp(timestamp) {
        if (!timestamp) return '—';
        try {
    ...
    refreshMacroButtons() {
        if (!Array.isArray(this.dom.macroButtons) || !this.dom.macroButtons.length) {
            return;
        }

        this.dom.macroButtons.forEach((button) => {
            const index = Number(button.dataset.macroSlot);
            if (!Number.isFinite(index)) return;
            const slot = this.macroManager.getSlot(index);
            const captured = Boolean(slot?.captured);
            button.dataset.captured = captured ? 'true' : 'false';
            button.classList.toggle('macro-btn--active', this.activeMacroSlot === index);
            if (this.activeMacroSlot !== index) {
                button.style.setProperty('--macro-progress', '0');
            }

            const label = slot?.label ?? `Macro ${index + 1}`;
            const timestamp = this.formatMacroTimestamp(slot?.updatedAt);
            button.innerHTML = `
                <span class="macro-btn__label">${label}</span>
                <span class="macro-btn__timestamp">${timestamp}</span>
            `;
        });
    }

    setMacroProgress(slotId, progress) {
        if (!Array.isArray(this.dom.macroButtons)) return;
        const clamped = Math.max(0, Math.min(1, Number(progress) || 0));
        this.dom.macroButtons.forEach((button) => {
            const index = Number(button.dataset.macroSlot);
            if (!Number.isFinite(index)) return;
            if (index === slotId) {
                button.classList.add('macro-btn--active');
                button.style.setProperty('--macro-progress', clamped.toFixed(3));
            } else if (this.activeMacroSlot !== index) {
                button.classList.remove('macro-btn--active');
                button.style.setProperty('--macro-progress', '0');
            }
        });
    }

    captureMacroSlot(index) {
        if (!this.macroManager) return;
        const snapshot = {
            vector: this.reactivityController.getLiveVector(),
            parameters: { ...this.lastReactiveParameters },
            audioMetrics: { ...this.lastAudioData }
        };

        const slot = this.macroManager.capture(index, snapshot, { timestamp: new Date().toISOString() });
        if (!slot) {
            this.updateMacroStatus('Unable to capture macro slot', 'error', 1600);
            return;
        }

        this.activeMacroSlot = index;
        this.currentMacroLabel = slot.label;
        this.refreshMacroButtons();
        this.setMacroProgress(index, 1);
        this.updateMacroStatus(`Stored ${slot.label}`, 'success', 1500);
    }

    recallMacroSlot(index) {
        if (!this.macroManager) return;
        const state = this.macroManager.recall(index, { vector: this.reactivityController.getLiveVector() }, this.getNow());
        if (!state) {
            this.updateMacroStatus('No macro stored in this slot yet', 'warning', 1600);
            return;
        }

        this.activeMacroSlot = index;
        this.currentMacroLabel = state.label;
        this.refreshMacroButtons();
        this.setMacroProgress(index, 0);
        this.updateMacroStatus(`Morphing to ${state.label}`, 'info', false);
        this.suspendAutomationPlayback(this.macroManager.getBlendDuration() + 200);
    }

    promptRenameMacro(index) {
        const slot = this.macroManager?.getSlot(index);
        if (!slot) return;
        const nextLabel = window.prompt('Rename macro slot', slot.label ?? `Macro ${index + 1}`);
        if (!nextLabel || !nextLabel.trim()) return;
        this.macroManager.renameSlot(index, nextLabel.trim());
        this.refreshMacroButtons();
        this.updateMacroStatus(`Renamed slot to ${nextLabel.trim()}`, 'success', 1400);
    }

    applyMacroTransition(timestamp) {
        if (!this.macroManager) return;
        const state = this.macroManager.update(timestamp);
        if (!state) return;

        this.activeMacroSlot = state.slotId;
        this.currentMacroLabel = state.label;
        this.setMacroProgress(state.slotId, state.easedProgress);
        this.commitLiveVector(state.vector, { updatePad: true });

        const statusMessage = state.isComplete
            ? `${state.label} locked in`
            : `Morphing → ${state.label} (${Math.round(state.progress * 100)}%)`;
        this.updateMacroStatus(statusMessage, state.isComplete ? 'success' : 'info', state.isComplete ? 1600 : false);
    ...
    updateMacroStatus(message, tone = 'info', clearAfter = 2000) {
        if (!this.dom.macroStatus) return;

        this.dom.macroStatus.textContent = message;
        this.dom.macroStatus.dataset.tone = tone;

        if (this.macroStatusTimeout) {
            clearTimeout(this.macroStatusTimeout);
            this.macroStatusTimeout = null;
        }

        if (clearAfter !== false) {
            this.macroStatusTimeout = setTimeout(() => {
                if (this.dom.macroStatus) {
                    this.dom.macroStatus.dataset.tone = 'muted';
                    this.dom.macroStatus.textContent = this.defaultMacroMessage;
                }
                this.macroStatusTimeout = null;
            }, clearAfter);
        }
    ...
    clearActiveMacro(label = 'Live Pad') {
        this.activeMacroSlot = null;
        this.currentMacroLabel = label;
        if (Array.isArray(this.dom.macroButtons)) {
            this.dom.macroButtons.forEach((button) => {
                button.classList.remove('macro-btn--active');
                button.style.setProperty('--macro-progress', '0');
    ...
    handleManualPadEngage() {
        if (this.macroManager) {
            this.macroManager.cancelTransition();
        }
        this.clearActiveMacro('Live Pad');
        this.updateMacroStatus('Manual pad control engaged', 'info', 1000);
    }
    ...
    updateInfoPanel(audioData) {
        ...
        if (this.dom.reactivityInfo) {
            ...
            const automationState = this.isAutomationPlaybackEnabled
                ...
                : 'OFF';
            const macroLabel = this.currentMacroLabel || 'Live Pad';
            this.dom.reactivityInfo.textContent = `Pad X: ${vector.x.toFixed(2)} | Y: ${vector.y.toFixed(2)} | Momentum: ${(audioData.momentum * 100).toFixed(0)}% | Macro: ${macroLabel} | Automation: ${automationState}`;
        }
    }
