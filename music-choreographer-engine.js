...
import { VisualizerLifecycleManager } from './src/core/VisualizerLifecycleManager.js';
import { createStorageBridge } from './src/core/StorageBridge.js';
...
const SCENE_STORAGE_SCHEMA_VERSION = 2;
...
export class MusicVideoChoreographer {
    constructor(mode = 'reactive') {
        ...
        this.visualizerLifecycle = null;
        this.storageBridge = createStorageBridge();
        this.storageBridgeSubscription = null;
        this.storageBridgeChangeSubscription = null;
        this.boundUnloadHandler = () => {
            this.teardown();
        };
        this.boundStorageRecovery = null;
        this.boundStorageOnline = null;
        this.boundStorageVisibility = null;
        ...
        const initialStorageAvailable = this.storageBridge.isAvailable();
        const initialStorageMode = this.storageBridge.getMode();
        const initialStoragePersistent = this.storageBridge.isPersistent();
        ...
        this.sceneState = {
            ...
            storagePersistent: initialStoragePersistent,
            storageFallbackAnnounced: false,
        ...
        this.automationSnapshotSignature = '';
    ...
    async init() {
        ...
        this.visualizerLifecycle = new VisualizerLifecycleManager({
            root: this.dom.visualizerContainer ?? document.getElementById('visualizer-container'),
            containerMap: {
        ...
        this.visualizerLifecycle?.showActiveContainer?.(this.currentSystem);

        this.configureReactivitySystem();
        this.initializeModulatorControls();
        this.initializeSceneLab();
        this.setupEventListeners();
        this.setupStorageRecoveryListeners();
    ...
    cacheDomReferences() {
        this.dom = {
            ...
            timelineQuantizeBtn: document.getElementById('timeline-quantize-btn'),
            timelineSnapBtn: document.getElementById('timeline-snap-btn'),
            timelineBeatHint: document.getElementById('timeline-beat-hint'),
            timelineBpmInput: document.getElementById('timeline-bpm-input'),
            timelineGridSelect: document.getElementById('timeline-grid-select')
        };
    }
    ...
    initializeSceneLab() {
        ...
        this.sceneState.storageAvailable = this.storageBridge.isAvailable();
        this.sceneState.storageMode = this.storageBridge.getMode();
        this.sceneState.storagePersistent = this.storageBridge.isPersistent();
        const storedScenePayload = this.storageBridge.get(this.sceneState.storageKey);
        this.sceneState.storageHasData = Boolean(storedScenePayload);
        this.sceneState.lastPersistedSignature = storedScenePayload || '';

        if (this.storageBridgeSubscription) {
            this.storageBridgeSubscription();
        }
        this.storageBridgeSubscription = this.storageBridge.subscribe(
            (mode) => this.handleStorageModeChange(mode),
            { fireImmediately: false }
        );
        if (this.storageBridgeChangeSubscription) {
            this.storageBridgeChangeSubscription();
        }
        this.storageBridgeChangeSubscription = this.storageBridge.subscribeChanges(
            (event) => this.handleStorageChange(event)
        );
        this.handleStorageModeChange(this.sceneState.storageMode);
    ...
    initializeSceneTimelineUi() {
        this.sceneTimelineUi = {
            ...
            quantizeBtn: this.dom.timelineQuantizeBtn,
            snapBtn: this.dom.timelineSnapBtn,
            beatHint: this.dom.timelineBeatHint,
            bpmInput: this.dom.timelineBpmInput,
            gridSelect: this.dom.timelineGridSelect
        };
        ...
        if (quantizeBtn) {
            quantizeBtn.addEventListener('click', () => this.quantizeTimelineCues());
        }

        if (snapBtn) {
            snapBtn.addEventListener('click', () => this.toggleTimelineSnapToBeat());
        }

        if (bpmInput) {
            bpmInput.addEventListener('change', (event) => this.setTimelineTempoOverride(event.target.value));
            bpmInput.addEventListener('keydown', (event) => {
                if (event.key === 'Enter') {
                    event.preventDefault();
                    this.setTimelineTempoOverride(event.target.value);
                }
            });
        }

        if (gridSelect) {
            gridSelect.addEventListener('change', (event) => this.setTimelineGridDivision(event.target.value));
        }
    ...
    updateScenePersistenceUi() {
        ...
        if (!this.sceneState.storageAvailable) {
            ...
            statusEl.dataset.mode = 'unavailable';
        } else if (!this.sceneState.storagePersistent) {
            statusEl.textContent = this.sceneState.scenes.length
                ? 'Session storage (clears on reload)'
                : 'Session storage active';
            statusEl.dataset.state = 'session';
            statusEl.dataset.mode = 'session';
    ...
    handleStorageModeChange(mode) {
        this.sceneState.storageAvailable = this.storageBridge.isAvailable();
        this.sceneState.storageMode = mode;
        this.sceneState.storagePersistent = mode === 'localStorage';

        if (!this.sceneState.storagePersistent && !this.sceneState.storageFallbackAnnounced) {
            this.sceneState.storageFallbackAnnounced = true;
            this.updateStatus('Persistent storage unavailable – using session-only saves.', 'warning');
        }

        if (this.sceneState.storagePersistent) {
            this.sceneState.storageFallbackAnnounced = false;
        }
    ...
    handleStorageChange(event) {
        ...
        if (type === 'clear' || !key || key === this.sceneState.storageKey) {
            this.handleSceneStorageEvent(event);
        }
        if (type === 'clear' || !key || key === AUTOMATION_SNAPSHOT_STORAGE_KEY) {
            this.handleAutomationStorageEvent(event);
        }
    }

    handleSceneStorageEvent(event) {
        if (!event || typeof event !== 'object') {
            return;
        }

        const targetKey = this.sceneState.storageKey;
        const {
            key,
            type,
            value,
        ...
        if (key && key !== targetKey && type !== 'clear') {
            return;
        }

        if (origin === 'local') {
            if (type === 'set') {
                this.sceneState.lastPersistedSignature = value || '';
                this.sceneState.storageHasData = Boolean(value);
            } else if (type === 'remove' || type === 'clear') {
                this.sceneState.lastPersistedSignature = '';
                this.sceneState.storageHasData = false;
            }
            return;
        }

        const nextMode = mode || this.storageBridge.getMode();
        this.sceneState.storageMode = nextMode;
        this.sceneState.storagePersistent = nextMode === 'localStorage';

        if (type === 'set' && value) {
            if (this.sceneState.lastPersistedSignature === value) {
                return;
            }
            const hadUnsavedChanges = this.sceneState.isDirty;
            const loaded = this.loadSceneSetFromStorage({
                announce: false,
                raw: value,
                origin: 'external'
            });
            if (loaded) {
                ...
                const message = hadUnsavedChanges
                    ? 'Scene set updated from another tab (local edits replaced)'
                    : 'Scene set updated from another tab';
                const statusType = hadUnsavedChanges ? 'warning' : 'info';
                this.updateStatus(message, statusType);
            }
            return;
        }

        if (type === 'remove' || type === 'clear' || !value) {
            const hadPersisted = Boolean(this.sceneState.lastPersistedSignature) || this.sceneState.storageHasData;
            if (!hadPersisted) {
                return;
            }
            ...
            this.renderSceneList();
            this.updateSceneUiState();
            this.updateStatus('Scene set cleared in another tab', 'warning');
        }
    }

    handleAutomationStorageEvent(event) {
        if (!event || typeof event !== 'object') {
            return;
        }
        ...
        if (key && key !== AUTOMATION_SNAPSHOT_STORAGE_KEY && type !== 'clear') {
            return;
        }

        if (origin === 'local') {
            if (type === 'set') {
                this.automationSnapshotSignature = value || '';
            } else if (type === 'remove' || type === 'clear') {
                this.automationSnapshotSignature = '';
            }
            return;
        ...
        if (type === 'set' && value) {
            if (this.automationSnapshotSignature === value) {
                return;
            }
            const snapshot = this.parseAutomationSnapshotRaw(value);
            if (!snapshot) {
                return;
            }
            ...
            this.automationSnapshotSignature = value;
            this.updateAutomationUiState();
            this.updateStatus('Automation snapshot updated from another tab', 'info');
            return;
        }

        if (type === 'remove' || type === 'clear' || !value) {
            if (!this.automationSnapshotSignature && !this.hasAutomationSnapshot()) {
                return;
            }
            this.automationSnapshot = null;
            this.automationSnapshotSignature = '';
            this.updateAutomationUiState();
            this.updateStatus('Automation snapshot cleared in another tab', 'warning');
        }
    }

    setupStorageRecoveryListeners() {
        if (typeof window === 'undefined') {
            return;
        }

        if (this.boundStorageRecovery || this.boundStorageOnline || this.boundStorageVisibility) {
            return;
        }

        const attemptRecovery = () => {
            if (!this.storageBridge || !this.storageBridge.isEphemeral()) {
                return;
            }
            const restored = this.storageBridge.tryRestorePersistence({ copyMemory: true });
            if (restored) {
                this.sceneState.storageAvailable = this.storageBridge.isAvailable();
                this.sceneState.storageMode = this.storageBridge.getMode();
                this.sceneState.storagePersistent = this.storageBridge.isPersistent();
                this.sceneState.storageHasData = Boolean(
                    this.storageBridge.get(this.sceneState.storageKey)
                );
                this.updateScenePersistenceUi();
                this.renderAutomationSnapshotInfo();
            }
        ...
        this.boundStorageRecovery = () => attemptRecovery();
        this.boundStorageOnline = () => attemptRecovery();
        this.boundStorageVisibility = () => {
            if (typeof document !== 'undefined' && document.visibilityState === 'visible') {
                attemptRecovery();
            }
        };

        window.addEventListener('focus', this.boundStorageRecovery, { passive: true });
        window.addEventListener('online', this.boundStorageOnline);
        if (typeof document !== 'undefined') {
            document.addEventListener('visibilitychange', this.boundStorageVisibility);
        }
    }
    ...
    persistSceneStateToStorage(options = {}) {
        ...
        const payload = {
            ...
            timeline: {
                ...
                snapToBeat: this.sceneState.timeline?.snapToBeat !== false,
                tempoOverride: Number.isFinite(this.sceneState.timeline?.tempoOverride)
                    && this.sceneState.timeline.tempoOverride > 0
                    ? Math.max(40, Math.min(240, Math.round(this.sceneState.timeline.tempoOverride)))
                    : null,
                gridDivision: Number.isFinite(this.sceneState.timeline?.gridDivision)
                    && this.sceneState.timeline.gridDivision > 0
                    ? this.sceneState.timeline.gridDivision
                    : 1
            }
        };

        if (!payload.scenes.length) {
            this.storageBridge.remove(this.sceneState.storageKey);
            this.sceneState.storageHasData = false;
            this.sceneState.lastSavedAt = null;
            this.sceneState.lastPersistedSignature = '';
        } else {
            const serialized = JSON.stringify(payload);
            this.storageBridge.set(this.sceneState.storageKey, serialized);
            this.sceneState.storageHasData = true;
            this.sceneState.lastSavedAt = payload.savedAt;
            this.sceneState.lastPersistedSignature = serialized;
        }
        ...
        if (announce) {
            const message = payload.scenes.length
                ? this.sceneState.storagePersistent
                    ? 'Scene set saved'
                    : 'Scene captured for this session'
                : 'Scene storage cleared';
            const statusType = payload.scenes.length && !this.sceneState.storagePersistent
                ? 'warning'
                : 'success';
            this.updateStatus(message, statusType);
        }
    ...
    loadSceneSetFromStorage(options = {}) {
        ...
        if (!this.sceneState.storageAvailable) {
            if (announce) {
                this.updateStatus('Storage unavailable', 'warning');
            }
            return null;
        }

        const hasProvidedRaw = Object.prototype.hasOwnProperty.call(options, 'raw');
        const raw = hasProvidedRaw ? providedRaw : this.storageBridge.get(this.sceneState.storageKey);
        ...
        try {
            ...
            this.sceneState.lastPersistedSignature = scenes.length ? raw : '';
            ...
            this.sceneState.timeline.snapToBeat = storedTimeline.snapToBeat !== false;
            const storedOverride = Number(storedTimeline.tempoOverride);
            this.sceneState.timeline.tempoOverride = Number.isFinite(storedOverride) && storedOverride > 0
                ? Math.max(40, Math.min(240, Math.round(storedOverride)))
                : null;
            const storedDivision = Number(storedTimeline.gridDivision);
            this.sceneState.timeline.gridDivision = Number.isFinite(storedDivision) && storedDivision > 0
                ? storedDivision
                : 1;
            ...
            if (announce) {
                const message = origin === 'external'
                    ? 'Scene set synchronised from another tab'
                    : 'Scene set loaded';
    ...
    formatTimelineBeatLabel(seconds = 0) {
        const bpm = this.getTimelineBPM();
        ...
        const beatValue = safeValue / baseInterval + 1;
        const decimals = beatValue < 10 ? 1 : beatValue < 100 ? 1 : 0;
        return `Beat ${beatValue.toFixed(decimals)}`;
    }
    ...
    renderSceneTimeline() {
        ...
        const snapEnabled = this.isTimelineSnapEnabled();
        const beatInterval = this.getTimelineBeatInterval();
        ui.list.innerHTML = '';
        ...
        cues.forEach((cue, index) => {
            ...
            const timeLabelText = this.formatTimelineTime(cue.time);
            const beatLabel = this.formatTimelineBeatLabel(cue.time);
            time.textContent = beatLabel
                ? `@ ${timeLabelText} • ${beatLabel}`
                : `@ ${timeLabelText}`;
            ...
            const timeLabelField = document.createElement('label');
            timeLabelField.textContent = 'Time (s)';
            const timeInput = document.createElement('input');
            timeInput.type = 'number';
            const stepValue = snapEnabled && beatInterval
                ? Math.max(0.01, Math.round(beatInterval * 1000) / 1000)
                : 0.1;
            timeInput.step = `${stepValue}`;
            timeInput.min = '0';
            timeInput.value = Number.isFinite(cue.time) ? cue.time : 0;
            timeInput.dataset.cueField = 'time';
            const inputBeatLabel = this.formatTimelineBeatLabel(cue.time);
            timeInput.title = inputBeatLabel ? `${inputBeatLabel} • seconds` : 'Cue time in seconds';
            timeLabelField.appendChild(timeInput);
            fields.appendChild(timeLabelField);
    ...
    updateSceneTimelineUiState() {
        ...
        const beatInterval = this.getTimelineBeatInterval();
        const bpm = this.getTimelineBPM();
        const usingOverride = Number.isFinite(timeline?.tempoOverride) && timeline.tempoOverride > 0;
        const hasDetectedTempo = Number.isFinite(this.detectedBPM) && this.detectedBPM > 0;
        const snapEnabled = this.isTimelineSnapEnabled();
        ...
        if (ui.quantizeBtn) {
            const hasGrid = Boolean(beatInterval);
            ui.quantizeBtn.disabled = !hasCues || !hasGrid;
            if (hasGrid) {
                const source = usingOverride
                    ...
                    : hasDetectedTempo
                        ? 'detected'
                        : 'fallback';
                const suffix = source === 'fallback' ? ' (fallback)' : '';
                ui.quantizeBtn.textContent = `🪄 Quantize ${Math.round(bpm)} BPM${suffix}`;
                const sourceDetail = suffix
                    ? ' using fallback tempo'
                    : source === 'manual'
                        ? ' using manual tempo'
                        : '';
                ui.quantizeBtn.title = `Snap cues to the ${Math.round(bpm)} BPM grid${sourceDetail}`;
            } else {
                ui.quantizeBtn.textContent = '🪄 Quantize Grid';
                ui.quantizeBtn.title = 'Enable tempo detection or set a manual BPM to quantize cues.';
            }
        }

        if (ui.snapBtn) {
            ui.snapBtn.classList.toggle('scene-timeline__toggle--active', snapEnabled);
            ui.snapBtn.setAttribute('aria-pressed', snapEnabled ? 'true' : 'false');
            ui.snapBtn.textContent = snapEnabled ? '🎚 Beat Snap' : '🎚 Free Timing';
            ui.snapBtn.title = snapEnabled
                ? 'Timeline cues will snap to the beat grid.'
                : 'Timeline cues use free timing.';
        }

        if (ui.bpmInput) {
            if (usingOverride) {
                ui.bpmInput.value = `${Math.round(timeline.tempoOverride)}`;
            } else {
                ui.bpmInput.value = '';
            }
            ui.bpmInput.placeholder = hasDetectedTempo ? `${Math.round(this.detectedBPM)}` : 'Auto';
            ui.bpmInput.title = hasDetectedTempo
                ? `Detected tempo ${Math.round(this.detectedBPM)} BPM (leave blank to follow)`
                : 'Enter a BPM override for the timeline grid';
        }

        if (ui.gridSelect) {
            const division = Number.isFinite(timeline?.gridDivision) && timeline.gridDivision > 0
                ? timeline.gridDivision
                : 1;
            const desired = `${division}`;
            const optionExists = Array.from(ui.gridSelect.options).some((option) => option.value === desired);
            ui.gridSelect.value = optionExists ? desired : '1';
            const divisionTitle = (() => {
                if (division === 0.5) return 'Grid division: every 2 beats';
                if (division === 1) return 'Grid division: every beat';
                if (division === 2) return 'Grid division: half-beats';
                if (division === 4) return 'Grid division: quarter-beats';
                return `Grid division: ${division}x subdivisions`;
            })();
            ui.gridSelect.title = divisionTitle;
        }
    ...
    updateSceneTimelineStatus(options = {}) {
        ...
        if (!timeline?.isArmed) {
            ...
            if (nextCue) {
                ...
                const beatLabel = this.formatTimelineBeatLabel(nextCue.time);
                const beatSegment = beatLabel ? ` • ${beatLabel}` : '';
                ui.status.textContent = `Timeline idle • Next: ${label} @ ${this.formatTimelineTime(nextCue.time)}${beatSegment}`;
            } else {
        ...
        const beatLabel = this.formatTimelineBeatLabel(nextCue.time);
        const beatSegment = beatLabel ? ` • ${beatLabel}` : '';
        ui.status.textContent = `Armed • ${label} in ${remaining.toFixed(1)}s${beatSegment}`;
        ui.status.dataset.state = 'armed';
    }

    isTimelineSnapEnabled() {
        return this.sceneState.timeline?.snapToBeat !== false;
    }

    getTimelineBPM() {
        const timeline = this.sceneState.timeline;
        if (Number.isFinite(timeline?.tempoOverride) && timeline.tempoOverride > 0) {
            return Math.max(40, Math.min(240, timeline.tempoOverride));
        }
        if (Number.isFinite(this.detectedBPM) && this.detectedBPM > 0) {
            return this.detectedBPM;
        }
        return 120;
    }

    getTimelineBeatInterval() {
        const bpm = this.getTimelineBPM();
        if (!Number.isFinite(bpm) || bpm <= 0) {
            return null;
        }
        const baseInterval = 60 / bpm;
        const division = Number.isFinite(this.sceneState.timeline?.gridDivision)
            && this.sceneState.timeline.gridDivision > 0
            ? this.sceneState.timeline.gridDivision
            : 1;
        return baseInterval / division;
    }

    roundTimelineTime(value, precision = 0.1) {
        const safeValue = Math.max(0, Number.isFinite(value) ? value : 0);
        const step = precision > 0 ? precision : 0.1;
        return Math.round(safeValue / step) * step;
    }

    snapTimeToTimelineBeat(value, options = {}) {
        const { strategy = 'nearest' } = options;
        const safeValue = Math.max(0, Number.isFinite(value) ? value : 0);
        const interval = this.getTimelineBeatInterval();
        if (!Number.isFinite(interval) || interval <= 0) {
            return this.roundTimelineTime(safeValue);
        }

        let multiplier;
        switch (strategy) {
            case 'floor':
                multiplier = Math.floor(safeValue / interval);
                break;
            case 'ceil':
                multiplier = Math.ceil(safeValue / interval);
                break;
            default:
                multiplier = Math.round(safeValue / interval);
                break;
        }

        const snapped = Math.max(0, multiplier * interval);
        return Math.max(0, Math.round(snapped * 1000) / 1000);
    }

    updateTimelineBeatHint() {
        const ui = this.sceneTimelineUi;
        if (!ui?.beatHint) {
            return;
        }
        ...
        const timeline = this.sceneState.timeline;
        if (!timeline) {
            ui.beatHint.textContent = '';
            return;
        }

        const snapEnabled = this.isTimelineSnapEnabled();
        const bpm = Math.round(this.getTimelineBPM());
        const usingOverride = Number.isFinite(timeline.tempoOverride) && timeline.tempoOverride > 0;
        const hasDetectedTempo = Number.isFinite(this.detectedBPM) && this.detectedBPM > 0;
        const source = usingOverride
            ? 'manual override'
        ...
        const division = Number.isFinite(timeline.gridDivision) && timeline.gridDivision > 0
            ...
            : 1;
        const divisionLabel = (() => {
            if (division === 0.5) return 'every 2 beats';
            if (division === 1) return 'every beat';
            if (division === 2) return 'half-beats';
            if (division === 4) return 'quarter-beats';
            return `${division}x subdivisions`;
        })();

        if (!snapEnabled) {
            ui.beatHint.textContent = `Free timing • grid ready @ ${bpm} BPM (${source})`;
            return;
        }

        ui.beatHint.textContent = `Beat snap @ ${bpm} BPM • ${divisionLabel} • ${source}`;
    }

    toggleTimelineSnapToBeat() {
        const timeline = this.sceneState.timeline;
        if (!timeline) {
            return;
        }

        const nextState = !this.isTimelineSnapEnabled();
        timeline.snapToBeat = nextState;
        this.markSceneStateDirty();

        let quantized = false;
        if (nextState && (timeline.cues?.length || 0) > 0) {
            quantized = this.quantizeTimelineCues({ announce: false });
        }
        ...
        const message = nextState
            ? `Beat snap enabled @ ${Math.round(this.getTimelineBPM())} BPM${quantized ? ' • cues aligned' : ''}`
            : 'Beat snap disabled – cues remain at manual timings';
        this.updateStatus(message, nextState ? 'success' : 'info');
    }

    quantizeTimelineCues(options = {}) {
        const { announce = true } = options;
        const timeline = this.sceneState.timeline;
        const cues = timeline?.cues || [];
        if (!cues.length) {
            if (announce) {
                this.updateStatus('No timeline cues to quantize', 'warning');
            }
            return false;
        }
        ...
        if (!Number.isFinite(interval) || interval <= 0) {
            if (announce) {
                this.updateStatus('Beat grid unavailable – set or detect tempo first', 'warning');
            }
            return false;
        }
        ...
        let changed = 0;
        cues.forEach((cue) => {
            const snapped = this.snapTimeToTimelineBeat(cue.time);
            if (Math.abs((cue.time ?? 0) - snapped) > 0.0005) {
                cue.time = snapped;
                changed += 1;
            }
        });
        ...
        if (!changed) {
            if (announce) {
                this.updateStatus('All cues already aligned to the beat grid', 'info');
            }
            return false;
        }
        ...
        if (announce) {
            const bpm = Math.round(this.getTimelineBPM());
            this.updateStatus(`Quantized ${changed} cue${changed === 1 ? '' : 's'} to ${bpm} BPM`, 'success');
        }

        return true;
    }

    setTimelineTempoOverride(value) {
        const timeline = this.sceneState.timeline;
        if (!timeline) {
            return;
        }
        ...
        if (!Number.isFinite(numeric) || numeric <= 0) {
            if (Number.isFinite(timeline.tempoOverride)) {
                timeline.tempoOverride = null;
                changed = true;
            }
        } else {
            const clamped = Math.max(40, Math.min(240, Math.round(numeric)));
            if (timeline.tempoOverride !== clamped) {
                timeline.tempoOverride = clamped;
                changed = true;
            }
        }
        ...
        const snapEnabled = this.isTimelineSnapEnabled();
        let quantized = false;
        if (snapEnabled && (timeline.cues?.length || 0) > 0) {
            quantized = this.quantizeTimelineCues({ announce: false });
        }
        ...
        if (changed) {
            const bpmValue = Number.isFinite(timeline.tempoOverride) && timeline.tempoOverride > 0
                ? Math.round(timeline.tempoOverride)
                : Math.round(this.getTimelineBPM());
            const message = Number.isFinite(timeline.tempoOverride) && timeline.tempoOverride > 0
                ? `Timeline grid overridden to ${bpmValue} BPM`
                : `Timeline grid following detected tempo (${bpmValue} BPM)`;
            this.updateStatus(message, 'info');
        }
    }

    setTimelineGridDivision(value) {
        const timeline = this.sceneState.timeline;
        if (!timeline) {
            return;
        }
        ...
        const numeric = Number(value);
        const division = Number.isFinite(numeric) && numeric > 0 ? numeric : 1;
        if (timeline.gridDivision === division) {
            this.updateSceneTimelineUiState();
            return;
        }

        timeline.gridDivision = division;
        this.markSceneStateDirty();
        ...
        this.updateStatus(`Timeline grid set to ${divisionLabel}`, 'info');
    }
    ...
    handleAddTimelineCue(options = {}) {
        ...
        const now = Number.isFinite(options.time)
            ? Math.max(0, options.time)
            : Math.max(0, this.audio?.currentTime ?? 0);
        const snappedTime = this.isTimelineSnapEnabled()
            ? this.snapTimeToTimelineBeat(now)
            : this.roundTimelineTime(now);
    ...
    handleTimelineFieldEdit(event) {
        ...
        const result = this.updateTimelineCue(cueId, field, value, { isInputEvent });
        if (!result.changed) {
            return;
        }
    ...
    updateTimelineCue(cueId, field, rawValue, context = {}) {
        const { skipSnap = false } = context;
        const timeline = this.sceneState.timeline;
        const cues = timeline?.cues || [];
        ...
        switch (field) {
            ...
            case 'time': {
                ...
                if (Number.isFinite(numeric)) {
                    let sanitized = Math.max(0, numeric);
                    if (this.isTimelineSnapEnabled() && !skipSnap) {
                        sanitized = this.snapTimeToTimelineBeat(sanitized);
                    } else {
                        sanitized = this.roundTimelineTime(sanitized);
                    }
                    sanitized = Math.max(0, Math.round(sanitized * 1000) / 1000);
                    if (Math.abs((cue.time ?? 0) - sanitized) > 0.0005) {
                        cue.time = sanitized;
                        result.changed = true;
                        result.needsResync = true;
    ...
    updateTimelineItemHeader(item, cue) {
        ...
        if (time) {
            const timeLabel = this.formatTimelineTime(cue.time);
            const beatLabel = this.formatTimelineBeatLabel(cue.time);
            time.textContent = beatLabel ? `@ ${timeLabel} • ${beatLabel}` : `@ ${timeLabel}`;
        }
    }
    ...
    snapCueToPlayhead(cueId) {
        ...
        const snapped = this.isTimelineSnapEnabled()
            ? this.snapTimeToTimelineBeat(currentTime)
            : this.roundTimelineTime(currentTime);
        const result = this.updateTimelineCue(cueId, 'time', snapped, { skipSnap: true });
        if (!result.changed) {
            return;
        }
    ...
    clearSceneTimeline(options = {}) {
        ...
        if (!preserveSettings) {
            ...
            timeline.snapToBeat = true;
            timeline.tempoOverride = null;
            timeline.gridDivision = 1;
        }
    ...
    parseAutomationSnapshotRaw(raw) {
        if (!raw) {
            return null;
        }
        ...
        try {
            ...
            console.warn('Failed to parse automation snapshot payload', error);
            return null;
        }
    }
    ...
    loadAutomationSnapshotFromStorage() {
        ...
        try {
            ...
            const snapshot = this.parseAutomationSnapshotRaw(raw);
            if (!snapshot) {
                this.automationSnapshotSignature = '';
                return null;
            }

            this.automationSnapshotSignature = raw;
            return snapshot;
        } catch (error) {
    ...
    persistAutomationSnapshot(snapshot) {
        ...
        try {
            ...
            storage.setItem(AUTOMATION_SNAPSHOT_STORAGE_KEY, serialized);
            this.automationSnapshotSignature = serialized;
        } catch (error) {
    ...
    renderAutomationSnapshotInfo() {
        ...
        if (!this.hasAutomationSnapshot()) {
            ...
            this.dom.automationSnapshotInfo.dataset.mode = this.sceneState.storagePersistent
                ? 'persistent'
                : 'session';
            return;
        }
        ...
        if (!this.sceneState.storagePersistent) {
            parts.push('Session only');
            this.dom.automationSnapshotInfo.dataset.mode = 'session';
        } else {
            this.dom.automationSnapshotInfo.dataset.mode = 'persistent';
        }
    ...
    async teardown() {
        ...
        if (this.pendingSystemSwitch) {
            try {
                ...
                console.warn('Pending system switch failed during teardown', error);
            }
        }
        ...
        if (this.storageBridgeSubscription) {
            try {
                ...
                console.warn('Failed to unsubscribe from storage bridge', error);
            }
            this.storageBridgeSubscription = null;
        }
        ...
        if (this.storageBridgeChangeSubscription) {
            try {
                ...
                console.warn('Failed to unsubscribe from storage change bridge', error);
            }
            this.storageBridgeChangeSubscription = null;
        }

        if (typeof document !== 'undefined') {
            document.removeEventListener('keydown', this.boundSceneHotkeyHandler);
            if (this.boundStorageVisibility) {
                document.removeEventListener('visibilitychange', this.boundStorageVisibility);
                this.boundStorageVisibility = null;
            }
        }
        if (typeof window !== 'undefined') {
            ...
            if (this.boundStorageRecovery) {
                window.removeEventListener('focus', this.boundStorageRecovery);
                this.boundStorageRecovery = null;
            }
            if (this.boundStorageOnline) {
                window.removeEventListener('online', this.boundStorageOnline);
                this.boundStorageOnline = null;
            }
        }
        ...
        if (this.xyPad && typeof this.xyPad.destroy === 'function') {
            try {
                this.xyPad.destroy();
            } catch (error) {
                console.warn('XY pad destroy failed', error);
            }
        }
        this.xyPad = null;

        if (this.visualizerLifecycle && typeof this.visualizerLifecycle.destroy === 'function') {
            try {
                await this.visualizerLifecycle.destroy();
            } catch (error) {
                console.warn('Visualizer lifecycle destroy failed', error);
            }
        }
        this.visualizerLifecycle = null;
        ...
        if (this.sourceNode) {
            try {
                ...
                console.warn('Source node disconnect failed', error);
            }
            this.sourceNode = null;
        }
    ...
    async switchSystem(systemName) {
        ...
        const performSwitch = async () => {
            if (!this.visualizerLifecycle || typeof this.visualizerLifecycle.switchSystem !== 'function') {
                throw new Error('Visualizer lifecycle manager unavailable');
            }

            const engineFactory = () => {
                if (systemName === 'faceted') {
                    return new VIB34DIntegratedEngine();
                }
                if (systemName === 'quantum') {
                    return new QuantumEngine();
                }
                if (systemName === 'holographic') {
                    return new RealHolographicSystem();
                }
            ...
            let engine;
            try {
                engine = await this.visualizerLifecycle.switchSystem(systemName, engineFactory);
            } catch (error) {
                console.error('Visualizer switch failed', error);
                this.updateStatus(`Failed to switch to ${systemName} system`, 'error');
                throw error;
            }
            ...
            this.applyLiveVector();
            this.visualizerLifecycle.ensureCanvasDimensions();
    ...
    updateInfoPanel(audioData) {
        if (this.dom.beatInfo) {
            const liveBpm = this.detectedBPM || '--';
            const snapEnabled = this.isTimelineSnapEnabled();
            const gridBpm = Math.round(this.getTimelineBPM());
            const gridSource = Number.isFinite(this.sceneState.timeline?.tempoOverride)
                && this.sceneState.timeline.tempoOverride > 0
                ? 'Manual'
                : Number.isFinite(this.detectedBPM) && this.detectedBPM > 0
                    ? 'Detected'
                    : 'Fallback';
            const gridLabel = snapEnabled
                ? `Snap @ ${gridBpm} BPM (${gridSource})`
                : `Free • ${gridBpm} BPM (${gridSource})`;
            this.dom.beatInfo.textContent = `BPM: ${liveBpm} | Beat Momentum: ${(this.reactivityController.getBeatMomentum() * 100).toFixed(0)}% | Timeline Grid: ${gridLabel}`;
        }
