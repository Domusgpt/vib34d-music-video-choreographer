...
export class MusicVideoChoreographer {
    ...
    async init() {
        ...
        this.initializeCanvasLifecycle();
    ...
    initializeCanvasLifecycle() {
        if (this.canvasLifecycle.initialized) {
            return;
        }
        ...
        const updateBounds = () => this.updateCanvasBounds();

        if (typeof ResizeObserver !== 'undefined') {
            const observer = new ResizeObserver(() => updateBounds());
            observer.observe(container);
            this.canvasLifecycle.resizeObserver = observer;
            this.registerCleanup(() => {
                observer.disconnect();
                this.canvasLifecycle.resizeObserver = null;
            });
        } else {
            const handler = () => updateBounds();
            window.addEventListener('resize', handler);
            this.canvasLifecycle.resizeHandler = handler;
            this.registerCleanup(() => {
                window.removeEventListener('resize', handler);
                this.canvasLifecycle.resizeHandler = null;
            });
        }

        if (window.visualViewport) {
            const handleViewport = () => updateBounds();
        ...
        this.registerCleanup(() => this.disposeCanvasSet());

        this.canvasLifecycle.initialized = true;
        updateBounds();
    }

    updateCanvasBounds() {
        const container = document.getElementById('vib34dLayers');
        if (!container || !this.canvasLifecycle.canvases.length) {
            return;
        }
        ...
        this.canvasLifecycle.canvases.forEach((canvas) => {
            if (!canvas) return;
    ...
    disposeCanvasSet() {
        ...
        const canvases = this.canvasLifecycle.canvases.length
            ? this.canvasLifecycle.canvases
        ...
        container.innerHTML = '';
        this.canvasLifecycle.canvases = [];
        this.canvasLifecycle.activeSystem = null;
    }

    createCanvasSet(systemName) {
        const container = document.getElementById('vib34dLayers');
        if (!container) {
            return [];
        }
        ...
        const canvases = canvasIds.map((id, index) => {
        ...
        this.canvasLifecycle.canvases = canvases;
        this.canvasLifecycle.activeSystem = systemName;
        this.updateCanvasBounds();
        return canvases;
    }

    getCanvasIdsForSystem(systemName) {
        const baseLayers = ['background', 'shadow', 'content', 'highlight', 'accent'];

        switch (systemName) {
            case 'quantum':
                return baseLayers.map((layer) => `quantum-${layer}-canvas`);
            case 'holographic':
                return baseLayers.map((layer) => `holo-${layer}-canvas`);
            case 'polychora':
                return baseLayers.map((layer) => `polychora-${layer}-canvas`);
            case 'faceted':
            default:
                return baseLayers.map((layer) => `${layer}-canvas`);
        }
    }
    ...
    setupEventListeners() {
        ...
        if (fileInput) {
            ...
            this.addDomListener(fileInput, 'change', handleFileChange);
        }

        if (playButton) {
            this.addDomListener(playButton, 'click', () => this.play());
        }
        if (pauseButton) {
            this.addDomListener(pauseButton, 'click', () => this.pause());
        }
        if (stopButton) {
            this.addDomListener(stopButton, 'click', () => this.stop());
        }
        ...
        if (timeline) {
            ...
            this.addDomListener(timeline, 'click', handleTimelineClick);
        }
        ...
        if (systemButtons && systemButtons.length) {
            systemButtons.forEach((btn) => {
                ...
                this.addDomListener(btn, 'click', handleClick);
            });
        }

        if (recordAutomationBtn) {
            this.addDomListener(recordAutomationBtn, 'click', () => this.toggleAutomationRecording());
        }

        if (exportAutomationBtn) {
            this.addDomListener(exportAutomationBtn, 'click', () => this.exportAutomationData());
        }

        if (automationPlaybackBtn) {
            this.addDomListener(automationPlaybackBtn, 'click', () => this.toggleAutomationPlayback());
        }

        if (importAutomationBtn) {
            this.addDomListener(importAutomationBtn, 'click', () => this.promptAutomationImport());
        }

        if (this.dom.lockXAxisBtn) {
            this.addDomListener(this.dom.lockXAxisBtn, 'click', () => this.toggleAxisLock('x'));
        }

        if (this.dom.lockYAxisBtn) {
            this.addDomListener(this.dom.lockYAxisBtn, 'click', () => this.toggleAxisLock('y'));
        }

        if (this.dom.resetPadBtn) {
            this.addDomListener(this.dom.resetPadBtn, 'click', () => this.resetLiveVector());
        }

        if (this.dom.resyncModulatorsBtn) {
            this.addDomListener(this.dom.resyncModulatorsBtn, 'click', () => this.handleResyncModulators());
        }

        if (automationGlideInput) {
            const handleGlideInput = (event) => {
            ...
            this.addDomListener(automationGlideInput, 'input', handleGlideInput);
        }

        if (saveAutomationSnapshotBtn) {
            this.addDomListener(saveAutomationSnapshotBtn, 'click', () => this.handleSaveAutomationSnapshot());
        }

        if (loadAutomationSnapshotBtn) {
            this.addDomListener(loadAutomationSnapshotBtn, 'click', () => this.handleLoadAutomationSnapshot());
        }

        this.addDomListener(this.audio, 'ended', () => this.stop());
        this.addDomListener(this.audio, 'timeupdate', () => this.updateTimeline());
    }
    ...
    initializeModulatorControls() {
        ...
        names.forEach((name) => {
            ...
            this.addDomListener(toggle, 'click', () => this.handleModulatorToggle(name));
    ...
    createModulatorSlider(container, options = {}) {
        ...
        this.addDomListener(slider, 'input', handleInput);
        ...
        if (typeof onCommit === 'function') {
            ...
            this.addDomListener(slider, 'change', handleChange);
        }
    ...
    initializeSceneLab() {
        ...
        if (this.sceneUi.captureBtn) {
            this.addDomListener(this.sceneUi.captureBtn, 'click', () => this.captureCurrentScene());
        }

        if (this.sceneUi.applyBtn) {
            this.addDomListener(this.sceneUi.applyBtn, 'click', () => this.launchSelectedScene());
        }

        if (this.sceneUi.saveBtn) {
            this.addDomListener(this.sceneUi.saveBtn, 'click', () => this.handleSceneSetSave());
        }

        if (this.sceneUi.loadBtn) {
            this.addDomListener(this.sceneUi.loadBtn, 'click', () => this.handleSceneSetLoad());
        }

        if (this.sceneUi.clearBtn) {
            this.addDomListener(this.sceneUi.clearBtn, 'click', () => this.handleSceneSetClear());
        }
        ...
        if (this.sceneUi.crossfadeInput) {
            ...
            const handleCrossfadeInput = (event) => {
            ...
            this.addDomListener(this.sceneUi.crossfadeInput, 'input', handleCrossfadeInput);
            this.updateSceneCrossfadeDisplay();
        }

        if (this.sceneUi.list) {
            this.addDomListener(this.sceneUi.list, 'click', (event) => this.handleSceneListClick(event));
            this.addDomListener(this.sceneUi.list, 'keydown', (event) => this.handleSceneListKeyDown(event));
        }
        ...
        this.addDomListener(document, 'keydown', this.boundSceneHotkeyHandler);
        this.addDomListener(window, 'beforeunload', this.boundBeforeUnloadHandler);
    }
    ...
    initializeSceneTimelineUi() {
        ...
        if (addCueBtn) {
            this.addDomListener(addCueBtn, 'click', () => this.handleAddTimelineCue());
        }

        if (clearBtn) {
            this.addDomListener(clearBtn, 'click', () => this.clearSceneTimeline({ announce: true }));
        }

        if (armBtn) {
            this.addDomListener(armBtn, 'click', () => this.toggleTimelineArmed());
        }

        if (followBtn) {
            this.addDomListener(followBtn, 'click', () => this.toggleTimelineFollow());
        }

        if (loopBtn) {
            this.addDomListener(loopBtn, 'click', () => this.toggleTimelineLoop());
        }

        if (quantizeBtn) {
            this.addDomListener(quantizeBtn, 'click', () => this.quantizeTimelineCues());
        }

        if (snapBtn) {
            this.addDomListener(snapBtn, 'click', () => this.toggleTimelineSnapToBeat());
        }

        if (bpmInput) {
            this.addDomListener(bpmInput, 'change', (event) => this.setTimelineTempoOverride(event.target.value));
            this.addDomListener(bpmInput, 'keydown', (event) => {
                if (event.key === 'Enter') {
        ...
        if (gridSelect) {
            this.addDomListener(gridSelect, 'change', (event) => this.setTimelineGridDivision(event.target.value));
        }

        if (list) {
            const handleFieldEdit = (event) => this.handleTimelineFieldEdit(event);
            this.addDomListener(list, 'input', handleFieldEdit);
            this.addDomListener(list, 'change', handleFieldEdit);
            this.addDomListener(list, 'click', (event) => this.handleTimelineListClick(event));
        }
    ...
    async switchSystem(systemName) {
        ...
        const performSwitch = async () => {
            ...
            this.disposeCanvasSet();
            const canvases = this.createCanvasSet(systemName);

            if (!canvases.length) {
                console.warn(`No canvases were created for system ${systemName}`);
                return;
            }
    ...
    async destroy() {
        ...
        if (this.pendingSystemSwitch) {
            try {
                ...
                console.warn('Pending system switch failed during destroy', error);
            }
        }
        ...
        try {
            ...
            console.warn('Failed to stop choreographer during destroy', error);
        }
        ...
        if (this.xyPad) {
            try {
                ...
                console.warn('Failed to destroy XY pad', error);
            }
            this.xyPad = null;
        }
        ...
        if (this.currentEngine && typeof this.currentEngine.destroy === 'function') {
            try {
                ...
                console.warn('Engine destroy failed during choreographer teardown', error);
            }
        }
        this.currentEngine = null;
        ...
        if (this.audio) {
            try {
                ...
                console.warn('Failed to reset audio element during destroy', error);
            }
        }
