...
export class LLMAudioChoreographer {
    ...
    setCoordinateSpaces(spaces = []) {
        if (!Array.isArray(spaces)) return;
        const sanitized = spaces
            .map(space => {
                if (!space || typeof space !== 'object') return null;
                const axes = {};
                COORDINATE_AXES.forEach(axisKey => {
                    const axis = space.axes?.[axisKey];
                    if (!axis || typeof axis !== 'object') return;
                    const inputs = Array.isArray(axis.inputs)
                        ? axis.inputs
                              .filter(input => input && typeof input === 'object')
                              .map(input => ({
                                  source: input.source === 'parameter' ? 'parameter' : 'audio',
                                  feature: input.feature,
                                  target: input.target,
                                  weight: typeof input.weight === 'number' ? input.weight : 1,
                                  transform: input.transform || 'linear',
                              }))
                        : [];
                    axes[axisKey] = {
                        label: axis.label ?? undefined,
                        bias: typeof axis.bias === 'number' ? axis.bias : 0,
                        normalize: Boolean(axis.normalize),
                        inputs,
                    };
                });
                return {
            ...
            .filter(Boolean);
        this.coordinateSpaces = sanitized;
        this.updateCoordinateProjections(true);
        if (sanitized.length > 0) {
            this.emit('log', {
                level: 'debug',
                message: `Coordinate spaces synced (${sanitized.length})`,
            });
        }
    }

    getCoordinateSpaces() {
        return this.coordinateSpaces.map(space => ({
            id: space.id,
            axes: space.axes,
    ...
    getCoordinateProjections() {
        return this.coordinateProjections.map(space => ({
            id: space.id,
            axes: space.axes,
            metadata: space.metadata,
            updatedAt: space.updatedAt,
    ...
    updateCoordinateProjections(force = false) {
        if (!force && this.coordinateSpaces.length === 0) {
            this.coordinateProjections = [];
            return;
        }
        this.coordinateProjections = this.coordinateSpaces.map(space => {
            const projection = {
                id: space.id,
                axes: {},
                metadata: space.metadata,
            ...
            COORDINATE_AXES.forEach(axisKey => {
                const axis = space.axes[axisKey];
                if (!axis) return;
                const bias = typeof axis.bias === 'number' ? axis.bias : 0;
                let accumulator = bias;
                let weightSum = 0;
                const inputSummaries = [];
                const inputs = Array.isArray(axis.inputs) ? axis.inputs : [];
                inputs.forEach(input => {
                    if (!input) return;
                    const weight = typeof input.weight === 'number' ? input.weight : 1;
                    let sample = 0;
                    if (input.source === 'parameter' && input.target && this.parameterManager) {
                        sample = this.parameterManager.getParameter(input.target) ?? 0;
                    } else if (input.source === 'audio' && input.feature) {
                        sample = this.features[input.feature] ?? 0;
                    }
                    switch (input.transform) {
                        case 'square':
                            ...
                            break;
                        case 'sqrt':
                            sample = Math.sqrt(Math.max(0, sample));
                            break;
                        case 'cube':
                    ...
                    inputSummaries.push({
                        source: input.source,
                        reference: input.feature ?? input.target ?? null,
                        weight,
                        transform: input.transform,
                        sample,
                    });
                });
                if (axis.normalize && weightSum > 0) {
                    accumulator = bias + (accumulator - bias) / weightSum;
                }
                projection.axes[axisKey] = {
                    value: accumulator,
                    label: axis.label,
    ...
    update(deltaSeconds = 0) {
        ...
        this.updateCoordinateProjections();
