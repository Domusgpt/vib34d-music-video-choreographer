...
export class DynamicControlEngine {
        ...
        intents.forEach(intent => {
            ...
            switch (sanitized.action) {
                ...
                case 'setCoordinateMap':
                    handlerResult = this.applyCoordinateMap(sanitized);
                    break;
                default:
    ...
    attachAudioChoreographer(choreographer) {
        ...
        if (typeof choreographer.setCoordinateSpaces === 'function') {
            choreographer.setCoordinateSpaces(this.getCoordinateSpaces());
        }
    }
    ...
    applyClearAudioBindings() {
        ...
        return {
            status: 'applied',
            message: 'Cleared all audio bindings',
            log: {
                level: 'info',
                message: 'Cleared all audio feature bindings',
            },
        };
    }

    applyCoordinateMap(intent) {
        const mapId = intent.mapId || 'default';
        const existing = this.coordinateMaps.get(mapId);
        const mergedAxes = { ...(existing?.axes ?? {}), ...(intent.axes ?? {}) };
        const snapshot = {
            axes: mergedAxes,
            metadata: intent.metadata !== undefined ? intent.metadata : existing?.metadata ?? null,
            updatedAt: Date.now(),
        };
        this.coordinateMaps.set(mapId, snapshot);
        if (this.audioChoreographer && typeof this.audioChoreographer.setCoordinateSpaces === 'function') {
            this.audioChoreographer.setCoordinateSpaces(this.getCoordinateSpaces());
        }
        const axisKeys = Object.keys(intent.axes || {});
        const axisSummary = axisKeys.length > 0 ? axisKeys.join('/') : 'none';
        const message = `Coordinate map ${mapId} updated (${axisSummary})`;
        const log = {
            level: 'info',
            message,
            detail: {
    ...
    getCoordinateSpaces() {
        return Array.from(this.coordinateMaps.entries()).map(([id, map]) => ({
            id,
            axes: map.axes,
        ...
        return {
            ...
            audio: this.audioChoreographer
                ? {
                      ...
                      coordinateSpaces: this.audioChoreographer.getCoordinateSpaces?.(),
                      projections: this.audioChoreographer.getCoordinateProjections?.(),
                  }
                : null,
            coordinateMaps: this.getCoordinateSpaces(),
        };
    }
}
