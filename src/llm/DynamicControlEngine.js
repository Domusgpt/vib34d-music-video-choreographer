...
export class DynamicControlEngine {
    ...
    applySetAutopilotState(intent) {
        ...
        if (intent.programId) {
            const outcome = this.autoPilot.setProgramState?.(intent.programId, intent.programEnabled);
            if (!outcome?.ok) {
                return {
                    status: 'rejected',
            ...
            const descriptor = outcome.enabled ? `Program enabled → ${label}` : `Program paused → ${label}`;
