...
export class LLMAgentAutoPilot {
        ...
        return [
            {
                ...
                build: context => {
                    ...
                    return {
                        smoothing: 0.34,
                        axes: {
                            x: { feature: 'energy', transform: 'sqrt', min: 0, max: 1.2 },
                            y: { feature: 'spectralCentroid', transform: 'linear', min: 0, max: 1.1 },
                        },
                        targets: [
                            {
                                target: 'saturation',
            ...
            },
            {
                ...
                build: context => {
                    ...
                    return {
                        smoothing: 0.4,
                        axes: {
                            x: { feature: 'bass', transform: 'sqrt', min: 0, max: 1.1 },
                            y: { feature: 'transient', transform: 'sqrt', min: 0, max: 1.1 },
                        },
                        targets: [
                            {
                                target: 'gridDensity',
            ...
            },
            {
                ...
                build: context => {
                    ...
                    return {
                        smoothing: 0.32,
                        axes: {
                            x: { feature: 'beat', transform: 'linear', min: 0, max: 1.3 },
                            y: { feature: 'energy', transform: 'sqrt', min: 0, max: 1.2 },
                        },
                        targets: [
                            {
                                target: 'intensity',
    ...
    maintainSurfaceMappings(now = performance.now(), options = {}) {
        ...
        this.surfacePlans.forEach(plan => {
            if (this.surfaceMappings.has(plan.key)) {
                return;
            }
