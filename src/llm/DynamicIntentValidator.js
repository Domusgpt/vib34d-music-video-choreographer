...
export function validateIntent(intent, parameterDefs = {}) {
    ...
    switch (sanitized.action) {
        ...
        case 'setAutopilotState': {
            ...
            if (!hasEnabled && !hasProgram) {
                errors.push('Autopilot intent requires an enabled flag or programId');
                break;
            }
            ...
            if (hasProgram) {
                ...
                if (sanitized.programEnabled === undefined) {
                    errors.push('Autopilot program intent missing programEnabled flag');
                } else {
