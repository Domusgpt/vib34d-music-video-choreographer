...
function parseWeightFromText(text) {
    ...
    if (/(heavy|strong|drive|push|dominant)/i.test(text)) {
        return 1.4;
    }
    if (/(light|subtle|gentle|feather)/i.test(text)) {
        return 0.6;
    }
    if (/(invert|negative|pull back|less)/i.test(text)) {
        return -1;
    }
    return 1;
}

function inferTransformFromText(text) {
    if (/(square|punch|hard|slam|staccato)/i.test(text)) {
        return 'square';
    }
    if (/(cube|extreme|wild|chaotic)/i.test(text)) {
        return 'cube';
    }
    if (/(smooth|gentle|soft|rounded|glide|ease)/i.test(text)) {
        return 'sqrt';
    }
    return 'linear';
}
...
function deriveAxisLabel(tokens) {
    const labels = tokens
        .map(token => {
            const feature = detectAudioFeature(token);
            if (feature) return feature.toUpperCase();
            const parameter = matchParameter(token);
            if (parameter) return parameter.toUpperCase();
            const normalized = normalizeLabelToken(token);
            return normalized ? normalized.replace(/\b(axis|with|vs|versus|against|and|normalize|normalized)\b/g, '').trim() : '';
        })
        .filter(Boolean);
    return labels.join(' vs ').trim();
}

function parseCoordinateMapCommand(segment, fallbackId = null) {
    if (!/(coordinate|axis|plane|quadrant|matrix)/i.test(segment)) {
        return null;
    }
    ...
    let axisCount = 0;
    ['x', 'y'].forEach(axisKey => {
        const axisRegex = new RegExp(`${axisKey}\\s*axis(?:\\s*(?:is|=|with|against|vs|versus|toward|towards))?\\s*([^.;]+)`, 'i');
        const axisMatch = segment.match(axisRegex);
        if (!axisMatch) return;
        const axisText = axisMatch[1].trim();
        if (!axisText) return;
        const tokens = axisText
            .split(/(?:vs|versus|against|with|and|\+|,)/i)
        ...
        tokens.forEach(token => {
            ...
            if (feature) {
                inputs.push({
                    source: 'audio',
                    feature,
                    weight: parseWeightFromText(token),
                    transform: inferTransformFromText(token),
                });
                return;
            }
            const parameter = matchParameter(token);
        ...
        const axisConfig = { inputs };
        const label = deriveAxisLabel(tokens);
        if (label) {
            axisConfig.label = label;
        }
        if (/normalize|normalized|balanced|blend/i.test(axisText)) {
            axisConfig.normalize = true;
        }
        const biasMatch = axisText.match(/bias(?:ed)?(?: towards| toward| to)?\s*(-?\d+(?:\.\d+)?)/i);
        if (biasMatch) {
            const bias = parseFloat(biasMatch[1]);
            if (!Number.isNaN(bias)) {
                axisConfig.bias = bias;
            }
        }
        axes[axisKey] = axisConfig;
        axisCount += 1;
    });
    ...
    let mapId = fallbackId || 'coordinate-plane';
    const nameMatch = segment.match(/(?:called|named|label(?:ed)?|title(?:d)?|as)\s+([a-z0-9\s-]+?)(?=\s+(?:with|featuring|using|for|x\s*axis|y\s*axis|and|$))/i);
    if (nameMatch) {
        const normalized = nameMatch[1].trim().toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-');
        if (normalized) {
            mapId = normalized.slice(0, 40);
        }
    } else if (/bass|low/.test(segment) && /(spark|high|treble|air|shine)/i.test(segment)) {
        mapId = 'spectrum-plane';
    }

    return { action: 'setCoordinateMap', mapId, axes };
}
...
export class LLMCommandRouter {
    ...
    processPrompt(prompt) {
        ...
        const axisSafePrompt = normalized
            .replace(/\band\s+(x\s*axis)/g, '; $1')
            .replace(/\band\s+(y\s*axis)/g, '; $1');
        const segments = axisSafePrompt.split(/(?:,| and | then |\n|;)/).map(seg => seg.trim()).filter(Boolean);
        segments.forEach(segment => {
            ...
            const coordinateMap = parseCoordinateMapCommand(segment, lastCoordinateMapId);
            if (coordinateMap) {
                intents.push(coordinateMap);
                notes.push(`Coordinate map parsed → ${coordinateMap.mapId}`);
                lastCoordinateMapId = coordinateMap.mapId;
                return;
            }
